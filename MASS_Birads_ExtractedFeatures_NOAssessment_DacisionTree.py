...
print("Loading JSON files for mass cases only...")
json_data = []
for json_file in os.listdir(json_folder_path):
    if json_file.endswith('.json') and 'mass' in json_file.lower():  # Filter for "mass" in file name
        with open(os.path.join(json_folder_path, json_file), 'r') as file:
            json_data.append(json.load(file))
json_df = pd.DataFrame(json_data)
print(f"Loaded {len(json_df)} JSON records related to mass cases.")
...
# Exclude unnecessary columns and assessment
columns_to_exclude = ['patient_id', 'pathology', 'image file path', 'cropped image file path', 'ROI mask file path', 'assessment']
