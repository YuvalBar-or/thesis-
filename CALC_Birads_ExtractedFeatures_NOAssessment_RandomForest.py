...
train_csv_path = 'csv/calc_case_description_train_set.csv'
test_csv_path = 'csv/calc_case_description_test_set.csv'
json_folder_path = 'BIRADS_JSON'  # Folder containing JSON files
...
# Drop 'assessment' if it exists
print("Dropping 'assessment' column if present...")
train_data_csv = train_data_csv.drop(columns=['assessment'], errors='ignore')
test_data_csv = test_data_csv.drop(columns=['assessment'], errors='ignore')
print("CSV files loaded and 'assessment' column excluded successfully!")

# Load JSON files and combine into a DataFrame (filter for "calc" cases)
print("Loading JSON files for calc cases only...")
json_data = []
for json_file in os.listdir(json_folder_path):
    if json_file.endswith('.json') and 'calc' in json_file.lower():  # Filter for "calc" in file name
        with open(os.path.join(json_folder_path, json_file), 'r') as file:
            json_data.append(json.load(file))
json_df = pd.DataFrame(json_data)
print(f"Loaded {len(json_df)} JSON records related to calc cases.")
