...
def load_all_datasets(csv_files, json_folder_path):
    train_labels = {}
    test_labels = {}
    for key, csv_path in csv_files.items():
        print(f"Processing {key} dataset: {csv_path}")
        if 'train' in key.lower():
            train_labels.update(load_pathology_labels(csv_path))
        else:
            test_labels.update(load_pathology_labels(csv_path))
    return train_labels, test_labels

# Load all pathology labels
train_labels, test_labels = load_all_datasets(csv_files, json_folder_path)
